...
export default async function MonitoringAssetDetailPage({ params }: PageProps) {
  ...
  if (hasHost && integration?.enabled) {
    try {
      const config = {
    ...
    <div className=space-y-6>
      ...
      <section className=rounded-lg border bg-card p-4 space-y-3>
        ...
        <div className=grid grid-cols-1 gap-4 md:grid-cols-2>
          ...
          <SummaryField label=Nombre técnico value={hostDetail?.host || monitoring.asset.monitoring?.monitoringHostname || monitoring.asset.hostname || —} />
          ...
          <SummaryField
            ...
            value={
              <span className={`inline-flex rounded px-2 py-1 text-xs font-medium ${getAvailabilityMeta(firstInterface?.available).className}`}>
                {getAvailabilityMeta(firstInterface?.available).label}
              </span>
            }
          />
          <SummaryField
      ...
      <section className=rounded-lg border bg-card p-4 space-y-3>
        ...
        <details className=rounded-md border p-3>
          ...
          <pre className=mt-3 max-h-96 overflow-auto rounded bg-muted p-4 text-xs font-mono>
            {JSON.stringify(hostDetail, null, 2)}
          </pre>
        </details>
